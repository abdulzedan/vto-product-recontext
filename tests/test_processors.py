...
from bulk_image_processor.analyzer import ClassificationResult, ImageCategory
from bulk_image_processor.downloader import ImageRecord
from bulk_image_processor.exceptions import ImageValidationError, ProcessingError
...
class TestBaseProcessor:
    ...
    class ConcreteProcessor(BaseProcessor):
        ...
        async def process_image(self, record, image_path, output_dir):
            return ProcessingResult(
                record_id=record.id, success=True, output_path=output_dir / "output.jpg"
            )
    ...
    def test_prepare_metadata(self, mock_settings, mock_analyzer):
        ...
        metadata = processor.prepare_metadata(
            record, additional_metadata={"custom_field": "value"}
        )
    ...
    async def test_process_with_retry_success(
        self, mock_settings, mock_analyzer, sample_image_record, tmp_path
    ):
        ...
        result = await processor.process_with_retry(
            sample_image_record, image_path, tmp_path
        )
    ...
    async def test_process_with_retry_failure(
        ...
        with patch("asyncio.sleep"):  # Mock sleep to speed up test
            result = await processor.process_with_retry(
                sample_image_record, tmp_path / "nonexistent.jpg", tmp_path
            )
...
class TestVirtualTryOnProcessor:
    ...
    def test_vto_processor_initialization(
        self, mock_load_images, mock_client, mock_init, mock_settings, mock_analyzer
    ):
    ...
    async def test_process_apparel_image_success(
        ...
        mock_analyzer.analyze_virtual_try_on_quality = AsyncMock(
            return_value=MagicMock(
                passed=True, score=0.92, reasoning="Good quality", suggestions=[]
            )
        )
        ...
        with patch.object(
            processor, "call_virtual_try_on", new_callable=AsyncMock
        ) as mock_call_api:
            ...
            with patch(
                "bulk_image_processor.processors.virtual_try_on.prediction_to_pil_image"
            ) as mock_convert:
                ...
                with patch.object(processor, "select_model") as mock_select:
                    ...
                    mock_analyzer.classify_image = AsyncMock(
                        return_value=MagicMock(
                            category="dress",
                            style="casual",
                    ...
                    with patch.object(
                        processor, "prepare_model_image"
                    ) as mock_prepare_model:
                        mock_prepare_model.return_value = "fake_model_image_bytes"

                        result = await processor.process_image(
                            sample_image_record, image_path, tmp_path / "output"
                        )
...
class TestProductRecontextProcessor:
    ...
    @patch("google.cloud.aiplatform.init")
    @patch("google.cloud.aiplatform.gapic.PredictionServiceClient")
    def test_product_processor_initialization(
        self, mock_client, mock_init, mock_settings, mock_analyzer
    ):
    ...
    async def test_process_product_image_success(
        ...
        mock_prediction.predictions = [{"bytesBase64Encoded": "fake_base64_image_data"}]
        mock_endpoint.predict.return_value = mock_prediction

        # Mock analyzer scene generation
        mock_analyzer.generate_scene_prompt = AsyncMock(
            return_value={
        ...
        # Mock analyzer feedback
        mock_analyzer.validate_output = AsyncMock(
            return_value={
                "pass": True,
                "score": 0.88,
        ...
        mock_analyzer.analyze_product_recontext_quality = AsyncMock(
            return_value=MagicMock(
                passed=True,
                score=0.88,
                reasoning="Good composition",
        ...
        # Mock the API call
        with patch.object(
            processor, "call_product_recontext", new_callable=AsyncMock
        ) as mock_call_api:
            # Mock API response
            mock_response = MagicMock()
            mock_response.predictions = [{"bytesBase64Encoded": "fake_base64_data"}]
            ...
            with patch(
                "bulk_image_processor.processors.product_recontext.prediction_to_pil_image"
            ) as mock_convert:
                mock_convert.return_value = img
    ...
    async def test_process_product_with_style_preference(
        ...
        with patch.object(
            ...
            with patch(
                ...
                result = await processor.process_image(
                    record, image_path, tmp_path / "output"
                )
        ...
        assert (
            len(call_args[0]) >= 2
        )  # Should have image_path and product_description arguments
