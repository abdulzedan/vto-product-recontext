...
class TestStorageManager:
    ...
    def test_initialization_gcs_enabled(
        self, mock_gcs_enabled_settings, mock_gcs_client
    ):
    ...
    def test_get_local_output_dir(self, mock_gcs_disabled_settings):
        ...
        with patch("bulk_image_processor.storage.ensure_directory"):
            ...
            expected = (
                mock_gcs_disabled_settings.storage.local_output_dir / "virtual_try_on"
            )
    ...
    def test_upload_file_success(
        self, mock_gcs_enabled_settings, mock_gcs_client, tmp_path
    ):
        ...
        with patch("bulk_image_processor.storage.ensure_directory"):
            ...
            result = storage_manager.upload_file(
                test_file, "uploads/test.txt", metadata={"source": "test"}
            )
    ...
    def test_save_processing_log_gcs_disabled(
        self, mock_gcs_disabled_settings, tmp_path
    ):
        ...
        with (
            ...
            expected_path = Path(
                "./logs/virtual_try_on_record_001_20240101_120000.json"
            )
            assert result == expected_path

            mock_file.assert_called_once_with(expected_path, "w")
            mock_json_dump.assert_called_once_with(
                log_data, mock_file(), indent=2, default=str
            )

    def test_save_processing_log_gcs_enabled(
        self, mock_gcs_enabled_settings, mock_gcs_client
    ):
    ...
    def test_create_processing_summary_gcs_disabled(self, mock_gcs_disabled_settings):
        ...
        with (
            ...
            mock_file.assert_called_once_with(expected_path, "w")
            mock_json_dump.assert_called_once_with(
                summary_data, mock_file(), indent=2, default=str
            )

    def test_create_processing_summary_gcs_enabled(
        self, mock_gcs_enabled_settings, mock_gcs_client
    ):
    ...
    def test_cleanup_old_files_directory_not_exists(
        self, mock_gcs_disabled_settings, tmp_path
    ):
    ...
    def test_get_storage_stats_gcs_enabled(
        self, mock_gcs_enabled_settings, mock_gcs_client
    ):
        ...
        with patch("bulk_image_processor.storage.ensure_directory"):
            ...
            assert (
                stats["gcs_storage"]["bucket"]
                == mock_gcs_enabled_settings.google_cloud.storage_bucket
            )

    def test_get_storage_stats_with_local_size(
        self, mock_gcs_disabled_settings, tmp_path
    ):
    ...
    def test_exception_handling_upload(
        self, mock_gcs_enabled_settings, mock_gcs_client, tmp_path
    ):
    ...
    def test_exception_handling_download(
        self, mock_gcs_enabled_settings, mock_gcs_client, tmp_path
    ):
    ...
    def test_exception_handling_list_files(
        self, mock_gcs_enabled_settings, mock_gcs_client
    ):
...
class TestStorageManagerErrorScenarios:
    ...
    def test_gcs_initialization_failure(self, mock_gcs_enabled_settings):
        ...
        with (
            patch(
                "google.cloud.storage.Client", side_effect=Exception("GCS init failed")
            ),
            patch("bulk_image_processor.storage.ensure_directory"),
        ):
    ...
    def test_upload_file_warning_on_gcs_failure(
        self, mock_gcs_enabled_settings, mock_gcs_client
    ):
    ...
    def test_storage_stats_size_calculation_error(
        self, mock_gcs_disabled_settings, tmp_path
    ):
