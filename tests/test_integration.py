...
class TestBulkImageProcessorIntegration:
    ...
    async def test_end_to_end_processing(
        ...
        gemini_instance.generate_content_async = AsyncMock(
        ...
        scene_response.text = (
            '{"prompt": "Luxury setting", "style": "luxury", "lighting": "soft"}'
        )
        ...
        mock_prediction.predictions = [{"bytesBase64Encoded": "fake_base64_output"}]
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "bulk_image_processor.storage.StorageManager._setup_storage"
                ) as mock_gcs_init:
    ...
    async def test_classification_routing(
        ...
        mock_records = [
            ...
            ImageRecord(
                "003", "https://example.com/unknown.jpg", "process", "front", 2
            ),
        ]
    ...
    @pytest.mark.asyncio
    async def test_concurrent_processing(self, mock_settings_integration, tmp_path):
        ...
        for i in range(num_images):
            data.append(
                [f"{i:03d}", f"https://example.com/image_{i}.jpg", "process", "front"]
            )
        ...
        async def mock_classify(*args, **kwargs):
            ...
            response = MagicMock()
            response.text = (
                '{"category": "PRODUCT", "confidence": 0.9, "reasoning": "Test"}'
            )
            return response

        with patch(
            "bulk_image_processor.downloader.ImageDownloader.download_single_image",
            new=mock_download,
        ):
            with patch("google.generativeai.GenerativeModel") as mock_gemini:
                with patch("google.cloud.aiplatform.Endpoint"):
                    ...
                    processor = BulkImageProcessor(mock_settings_integration)
                    processor.settings.processing.max_workers = (
                        5  # Allow concurrent operations
                    )
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown(self, mock_settings_integration, sample_csv_file):
        ...
        with patch("google.generativeai.GenerativeModel"):
            with patch("google.cloud.aiplatform.Endpoint"):
                ...
                with patch(
                    "bulk_image_processor.downloader.download_images_from_csv"
                ) as mock_dl:
    ...
    @pytest.mark.asyncio
    async def test_storage_integration(self, mock_settings_integration, tmp_path):
