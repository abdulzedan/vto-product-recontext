...
class TestBulkImageProcessor:
    ...
    def mock_components(self):
        ...
        with (
            patch("bulk_image_processor.main.GeminiAnalyzer") as mock_analyzer_class,
            patch("bulk_image_processor.main.VirtualTryOnProcessor") as mock_vto_class,
            patch(
                "bulk_image_processor.main.ProductRecontextProcessor"
    ...
    async def test_classify_single_image_success(self, mock_settings, mock_components):
        ...
        mock_components["analyzer"].classify_image = AsyncMock(
        ...
        mock_components["analyzer"].classify_image.assert_called_once_with(
            image_path, additional_context="Command: process, Position: front"
        )
    ...
    async def test_classify_images_parallel(self, mock_settings, mock_components):
        ...
        processor._classify_single_image = AsyncMock(
            side_effect=[ImageCategory.APPAREL, ImageCategory.PRODUCT]
        )
    ...
    async def test_process_classified_images(self, mock_settings, mock_components):
        ...
        classified_images = [
            (
                ImageRecord(
                    id="001",
                    image_url="https://example.com/apparel.jpg",
                    image_command="cmd1",
                    image_position="pos1",
                    row_index=0,
                ),
                Path("/apparel.jpg"),
                ImageCategory.APPAREL,
            ),
            (
                ImageRecord(
                    id="002",
        ...
        processor._process_with_processor = AsyncMock(
            side_effect=[
    ...
    async def test_process_with_processor(self, mock_settings, mock_components):
        ...
        mock_processor.process_with_retry = AsyncMock(
            side_effect=[
        ...
        mock_components["storage_manager"].get_local_output_dir.return_value = Path(
            "/output"
        )
        ...
        with patch("bulk_image_processor.main.ProgressTracker") as mock_progress:
            ...
            result = await processor._process_with_processor(
                images, mock_processor, "test_processor"
            )
    ...
    def test_create_summary(self, mock_settings, mock_components):
        ...
        mock_components["vto_processor"].get_stats.return_value = {
            "total": 5,
            "success": 4,
        }
        mock_components["product_processor"].get_stats.return_value = {
            "total": 3,
            "success": 3,
        }
        mock_components["storage_manager"].get_storage_stats.return_value = {
            "uploads": 7
        }
        ...
        summary = processor._create_summary(
            start_time, download_results, classified_images, processing_results
        )
    ...
    def test_get_system_status_uninitialized(self, mock_settings):
        ...
        with (
            patch("bulk_image_processor.main.setup_logging"),
            patch(
                "bulk_image_processor.main.GeminiAnalyzer",
...
class TestMainFunction:
    ...
    async def test_main_with_csv_path(self, tmp_path):
        ...
        with patch(
            "bulk_image_processor.main.BulkImageProcessor"
        ) as mock_processor_class:
            mock_processor = MagicMock()
            mock_processor_class.return_value = mock_processor
            mock_processor.process_from_csv = AsyncMock(
