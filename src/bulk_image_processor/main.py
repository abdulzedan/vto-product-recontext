...
class BulkImageProcessor:
    ...
    async def process_from_csv(self, csv_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            successful_downloads = [
                (record, path) for record, path in download_results if path is not None
            ]
            ...
            processing_results = await self._process_classified_images(
                classified_images, run_path
            )
    ...
    async def _classify_images(
        ...
        ) -> Tuple[ImageRecord, Path, ImageCategory]:
        ...
        tasks = [
            classify_with_semaphore(record, path) for record, path in download_results
        ]
    ...
    async def _classify_single_image(
        self, record: ImageRecord, path: Path
    ) -> ImageCategory:
    ...
    async def _process_classified_images(
        ...
        apparel_images = [
            (record, path)
            for record, path, category in classified_images
    ...
    async def _process_with_processor(
            ...
            async with semaphore:
                ...
                try:
                    result = await processor.process_with_retry(
                        record, path, output_dir
                    )
                    progress.update(success=result.success)
                    return result
                except Exception as e:
                    logger.error(
                        "Processing task failed", record_id=record.id, error=str(e)
                    )
                    progress.update(success=False)
                    return ProcessingResult(
                        record_id=record.id,
        ...
        tasks = [process_with_semaphore(record, path) for record, path in images]

        logger.info(
            f"Starting parallel processing of {len(tasks)} images with {processor_name}"
        )
        results = await asyncio.gather(*tasks, return_exceptions=False)
    ...
    def _create_summary(
        ...
        classification_stats = {
            ...
            "apparel_count": sum(
                1 for _, _, cat in classified_images if cat == ImageCategory.APPAREL
            ),
            "product_count": sum(
                1 for _, _, cat in classified_images if cat == ImageCategory.PRODUCT
            ),
            "unknown_count": sum(
                1 for _, _, cat in classified_images if cat == ImageCategory.UNKNOWN
            ),
        }
        ...
        # Quality statistics
        quality_scores = [
            r.quality_score for r in processing_results if r.quality_score is not None
        ]
        quality_stats = {
    ...
    async def _upload_run_directory_to_gcs(self, run_path: Path) -> None:
        ...
        try:
            ...
            logger.info(
                "GCS client and bucket initialized",
                bucket_name=self.settings.google_cloud.storage_bucket,
            )
    ...
    def get_system_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            "processors": {
                "vto_stats": (
                    self.vto_processor.get_stats() if self.vto_processor else None
                ),
                "product_stats": (
                    self.product_processor.get_stats()
                    if self.product_processor
                    else None
                ),
            },
            "storage": (
