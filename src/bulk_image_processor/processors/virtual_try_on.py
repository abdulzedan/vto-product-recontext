...
class VirtualTryOnProcessor(BaseProcessor):
    ...
    def select_model(
        self, apparel_info: Dict[str, Any], exclude_models: List[str] = None
    ) -> Dict[str, Any]:
        ...
        if target_gender == "unisex":
            ...
            all_models = self.models_by_gender.get(
                "woman", []
            ) + self.models_by_gender.get("man", [])
        ...
        if target_gender and target_gender in self.models_by_gender:
            suitable_models = [
                m
                for m in self.models_by_gender[target_gender]
        ...
        if any(
            item.lower() in ["dress", "blouse", "skirt", "heels"]
        ...
        if preferred_gender and preferred_gender in self.models_by_gender:
            suitable_models = [
                m
                for m in self.models_by_gender[preferred_gender]
                if m["id"] not in exclude_models
            ]
            if suitable_models:
                selected_model = random.choice(suitable_models)
        ...
        logger.error(
            ...
            total_models_per_gender={
                g: len(models) for g, models in self.models_by_gender.items()
            },
        )
    ...
    async def process_image(
        ...
        while total_attempts < max_total_attempts:
            ...
            if result.success and hasattr(result, "metadata") and result.metadata:
                ...
                current_model = result.metadata.get("selected_model", {}).get(
                    "id", "unknown"
                )
                ...
                if quality_feedback and hasattr(quality_feedback, "garment_applied"):
                    ...
                    if not garment_applied:
                        ...
                        if retry_count < max_api_retries_per_model:
                            ...
                            failed_attempt_file = (
                                record_output_dir
                                / f"failed_attempt_{total_attempts}.json"
                            )
                            failed_attempt_image = (
                                record_output_dir
                                / f"failed_attempt_{total_attempts}.jpg"
                            )
                            ...
                            if hasattr(result, "_temp_result_data"):
                                ...
                                failed_image.save(
                                    failed_attempt_image, "JPEG", quality=95
                                )
                            ...
                            failed_attempt_data = {
                                ...
                                "gemini_interpretation": (
                                    quality_feedback.reasoning
                                    if hasattr(quality_feedback, "reasoning")
                                    else "No reasoning provided"
                                ),
                                "failed_image": (
                                    str(failed_attempt_image)
                                    if failed_attempt_image.exists()
                                    else None
                                ),
                            }
                        ...
                        failed_attempt_file = (
                            record_output_dir / f"failed_attempt_{total_attempts}.json"
                        )
                        failed_attempt_image = (
                            record_output_dir / f"failed_attempt_{total_attempts}.jpg"
                        )
                        ...
                        logger.warning(
                            ...
                            model_id=current_model,
                            length_issue=getattr(
                                quality_feedback, "length_issue", "Length mismatch"
                            ),
                            quality_score=getattr(result, "quality_score", 0.0),
                            total_attempt=total_attempts,
                            failure_saved=str(failed_attempt_file),
        ...
        if best_result is not None:
            ...
            if hasattr(best_result, "_temp_result_data"):
                ...
                if best_result.metadata:
                    ...
                    best_result.metadata["confidence_reason"] = (
                        "Failed quality checks but best available"
                    )
    ...
    async def _save_successful_result(
        self, result: ProcessingResult, record: ImageRecord, image_path: Path
    ) -> None:
    ...
    async def _process_with_model(
        ...
        try:
            ...
            temp_metadata = self.prepare_metadata(
                record,
                {
                    ...
                    "fashion_coordination": getattr(
                        selected_model, "fashion_reasoning", None
                    ),
                },
            )
