...
class PipelineBulkImageProcessor:
    ...
    def _setup_components(self) -> None:
        ...
        try:
            ...
            self.product_processor = ProductRecontextProcessor(
                self.settings, self.analyzer
            )
    ...
    async def process_from_csv(self, csv_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(
                    self._download_stage(
                        csv_path, run_path, download_queue, all_downloads
                    )
                ),
                asyncio.create_task(
                    self._classification_stage(
                        download_queue, vto_queue, product_queue, all_classifications
                    )
                ),
                asyncio.create_task(
                ...
                asyncio.create_task(self._progress_monitor(total_images)),
            ]
            ...
            statistics = generate_run_statistics(
                run_path, results_for_stats, total_duration
            )

            # Calculate pipeline-specific metrics
            if self.pipeline_stats["first_result_time"]:
                time_to_first_result = (
                    self.pipeline_stats["first_result_time"] - start_time
                )
            else:
            ...
            summary = {
                ...
                "pipeline_metrics": {
                    "time_to_first_result": (
                        round(time_to_first_result, 2) if time_to_first_result else None
                    ),
                    "downloads_completed": self.pipeline_stats["downloads_completed"],
                    "classifications_completed": self.pipeline_stats[
            ...
            logger.info(
                ...
                total_processed=len(all_results),
                time_to_first_result=(
                    round(time_to_first_result, 2) if time_to_first_result else "N/A"
                ),
            )
    ...
    async def _download_stage(
        self, csv_path: Path, run_path: Path, output_queue: Queue, results_list: List
    ):
        ...
        async with ImageDownloader(self.settings) as downloader:
            ...
            async def download_and_queue(record: ImageRecord):
                ...
                async with semaphore:
                    try:
                        path = await downloader.download_single_image(
                            record, downloads_dir
                        )
                        if path:
                        ...
                        logger.error(
                            f"Download failed for {record.id}: {error_type} - {e}"
                        )
            ...
            if failed_downloads:
                ...
                logger.info(
                    f"Download stage completed: {self.pipeline_stats['downloads_completed']}/{total_records} images"
                )
    ...
    async def _classification_stage(
        self,
        input_queue: Queue,
        vto_queue: Queue,
        product_queue: Queue,
                ...
                async with semaphore:
                    try:
                        ...
                        if category == ImageCategory.APPAREL:
                            ...
                            await product_queue.put(
                                (record, path, category, apparel_info)
                            )
                        ...
                        await product_queue.put(
                            (record, path, ImageCategory.UNKNOWN, {})
                        )
                        self.pipeline_stats["classifications_completed"] += 1
        ...
        logger.info(
            f"Classification stage completed: {self.pipeline_stats['classifications_completed']} images"
        )

    async def _vto_processing_stage(
        self, vto_queue: Queue, run_path: Path, results_list: List
    ):
        """VTO processing stage - processes apparel images through Virtual Try-On."""
        logger.info("Starting VTO processing stage")

        vto_semaphore = asyncio.Semaphore(
            min(self.settings.processing.max_workers // 2, 15)
        )
    ...
    async def _product_processing_stage(
        self, product_queue: Queue, run_path: Path, results_list: List
    ):
        ...
        product_semaphore = asyncio.Semaphore(
            min(self.settings.processing.max_workers, 25)
        )
        ...
        while not self.shutdown_requested:
            ...
            if current_progress == last_progress:
                ...
                if stall_counter >= 6:  # No progress for 60 seconds
                    ...
                    if (
                        current_progress["downloads"] < total_images
                        and current_progress["processing"]
                        == current_progress["downloads"]
                    ):
                        logger.warning(
                        ...
                        if (
                            ...
                            == current_progress["classifications"]
                            == current_progress["downloads"]
                        ):
                            logger.info(
                                "All available items processed. Exiting pipeline."
                            )
                            break
            else:
                stall_counter = 0
            ...
            if (
                ...
                == current_progress["processing"]
                and stall_counter >= 3
            ):  # Give it 30 seconds to ensure everything is done
                logger.info(
    ...
    async def _upload_run_directory_to_gcs(self, run_path: Path) -> None:
        ...
        try:
            ...
            logger.info(
                f"Found {len(files_to_upload)} files to upload, total size: {total_size / (1024*1024):.2f} MB"
            )

            # Upload files concurrently
            async def upload_file_async(
                local_path: Path, gcs_path: str, semaphore: asyncio.Semaphore
            ) -> bool:
                async with semaphore:
                    try:
                        ...
                        await loop.run_in_executor(
                            None,
                            lambda: bucket.blob(gcs_path).upload_from_filename(
                                str(local_path)
                            ),
                        )
                        return True
